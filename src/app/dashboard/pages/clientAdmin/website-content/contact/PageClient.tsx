...
const ContactAdminDashboard = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (changedContactFields.size > 0) {
      ...
      didPublish = didPublish || success;
    }

    if (changedAppointmentFields.size > 0) {
      const success = await saveAppointmentFormData();
      didPublish = didPublish || success;
    }
      ...
      for (const part of parts) {
        if (Array.isArray(current) && /^\d+$/.test(part)) {
          ...
          if (index >= 0 && index < current.length) {
            ...
            console.error("Invalid array index encountered");
            return newData;
          }
        } else if (
  ...
  const addContactArrayItem = <T,>(arrayPath: string, newItem: T) => {
    setContactFormData((prev: ContactContent) => {
      ...
      if (
        typeof current === "object" &&
        current !== null &&
        Array.isArray(currentRecord[lastPart])
      ) {
        currentRecord[lastPart] = [...(currentRecord[lastPart] as unknown[]), newItem];
      }
